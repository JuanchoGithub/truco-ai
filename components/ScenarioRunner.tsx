...
import { Card, GameState, AiMove } from '../types';
...
type ExpectedActionDistribution = Record<string, number>;

interface ExpectedScenarioResult {
    scenario: string;
    totalRuns: number;
    actionDistribution: ExpectedActionDistribution;
}
...
const StatusIcon: React.FC<{ status: ValidationStatus }> = ({ status }) => {
    const statusMap: Record<ValidationStatus, { icon: React.ReactNode; color: string }> = {
        ok: { icon: <path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />, color: 'text-green-400' },
        warn: { icon: <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />, color: 'text-yellow-400' },
        fail: { icon: <path strokeLinecap="round" strokeLinejoin="round" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z" />, color: 'text-red-400' },
        pending: { icon: <path strokeLinecap="round" strokeLinejoin="round" d="M8.228 9c.549-1.165 2.03-2 3.772-2 2.21 0 4 1.343 4 3 0 1.4-1.278 2.575-3.006 2.907-.542.104-.994.54-.994 1.093m0 3h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />, color: 'text-gray-500' },
    };
    return (
        <svg xmlns="http://www.w3.org/2000/svg" className={`h-6 w-6 ${statusMap[status].color}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
...
const defaultExpectedResultsData = {
  "scenarios": [
    { "scenario": "scenario_tester.scenario_names.parda_y_gano", "totalRuns": 1000, "actionDistribution": { "call_truco_value": 390, "call_truco_parda_y_gano": 239, "secure_hand": 359, "call_truco_post_parda_bluff": 11, "call_truco_bluff": 1 } },
    ...
    { "scenario": "scenario_tester.scenario_names.lopsided_bait", "totalRuns": 1000, "actionDistribution": { "call_real_dominant": 488, "secure_hand": 303, "probe_envido_hide": 209 } },
    ...
    { "scenario": "scenario_tester.scenario_names.flor_vs_envido", "totalRuns": 1000, "actionDistribution": { "unknown_action": 1000 } },
    ...
    { "scenario": "scenario_tester.scenario_names.dual_brava_probe", "totalRuns": 1000, "actionDistribution": { "call_envido_strong": 358, "probe_low_value": 520, "call_truco_value": 96, "call_envido_marginal": 26 } },
    ...
    { "scenario": "scenario_tester.scenario_names.signal_masked_depletion", "totalRuns": 1000, "actionDistribution": { "probe_low_value": 847, "call_truco_value": 153 } },
    ...
    { "scenario": "scenario_tester.scenario_names.triple_brava_figure_probe", "totalRuns": 1000, "actionDistribution": { "call_truco_value": 166, "probe_low_value": 834 } },
    ...
    { "scenario": "scenario_tester.scenario_names.late_false_ace_chain", "totalRuns": 1000, "actionDistribution": { "probe_low_value": 854, "call_truco_value": 146 } },
    ...
    { "scenario": "scenario_tester.scenario_names.ancho_deplete_post_tie", "totalRuns": 1000, "actionDistribution": { "play_last_card": 645, "call_truco_value": 155, "call_truco_certain_win": 200 } },
    ...
    { "scenario": "scenario_tester.scenario_names.mid_low_signal_mask", "totalRuns": 1000, "actionDistribution": { "call_real_dominant": 201, "call_envido_strong": 377, "call_envido_bluff": 71, "secure_hand": 336, "call_truco_value": 13, "call_truco_bluff": 2 } },
    ...
    { "scenario": "scenario_tester.scenario_names.endgame_low_balance", "totalRuns": 1000, "actionDistribution": { "call_truco_bluff": 106, "call_falta_win_game": 169, "call_real_dominant": 42, "call_envido_bluff": 161, "probe_mid_value": 179, "call_envido_strong": 281, "secure_hand": 58, "call_truco_value": 4 } },
    ...
    { "scenario": "scenario_tester.scenario_names.dual_ancho_bait", "totalRuns": 1000, "actionDistribution": { "probe_low_value": 723, "call_truco_value": 199, "call_envido_marginal": 78 } },
    ...
    { "scenario": "scenario_tester.scenario_names.clutch_false_deplete", "totalRuns": 1000, "actionDistribution": { "probe_mid_value": 767, "secure_hand": 233 } }
  ]
};
...
const ScenarioResultRow: React.FC<{
    ...
    const validation = useMemo(() => {
        ...
        // Fix: Explicitly cast sort operands to Number to prevent potential type errors during arithmetic operation.
        ...
        for (const [action, expectedCount] of Object.entries(expected.actionDistribution)) {
            // Fix: Explicitly cast `expectedCount` to Number to prevent a type error where it might be inferred incorrectly in complex scenarios.
            ...
            const allowedDeviation = expectedFreq * (deviation / 100);

            if (Math.abs(actualFreq - expectedFreq) > allowedDeviation) {
                hasDeviation = true;
                break;
            }
        }
    ...
    const topActionFreq = result.totalRuns > 0 && validation.actualTopAction ? ((result.results[validation.actualTopAction] / result.totalRuns) * 100).toFixed(1) + '%' : '0%';
    const expectedTopActionFreq = expected && validation.expectedTopAction ? ((expected.actionDistribution[validation.expectedTopAction] / expected.totalRuns) * 100).toFixed(1) + '%' : 'N/A';
    
    const allActions = new Set([...Object.keys(result.results), ...(expected ? Object.keys(expected.actionDistribution) : [])]);
    const sortedActions = Array.from(allActions).sort((a, b) => (result.results[b] || 0) - (result.results[a] || 0));
            ...
            <div className="bg-black/30 p-4">
                <table className="w-full text-xs">
                    ...
                    {sortedActions.map(key => {
                        ...
                        const deviationAmount = actualFreq - expectedFreq;
                        const allowedDeviation = expectedFreq * (deviation / 100);
                        const isOutlier = Math.abs(deviationAmount) > allowedDeviation;
                        ...
                        return (
                            <tr key={key} className="border-t border-cyan-900/50">
                                ...
                                <td className={`py-1 text-right font-mono ${deviationColor}`}>{deviationAmount > 0 ? '+' : ''}{(deviationAmount * 100).toFixed(1)}%</td>
                            </tr>
                        );
                    })}
                    </tbody>
...
const ScenarioRunner: React.FC = () => {
    ...
    const [copyStatus, setCopyStatus] = useState(t('scenario_runner.copy_button'));
    const [exportStatus, setExportStatus] = useState(t('scenario_runner.export_button'));
    ...
    const handleCopy = () => { /* ... (implementation as before) ... */ };
    const handleExport = () => { /* ... (implementation as before) ... */ };
        ...
        <div className="w-full h-full flex flex-col gap-4 text-white">
            <div className="flex-shrink-0 bg-black/30 p-4 rounded-lg border border-cyan-700/50 space-y-3">
                ...
                <div className="flex items-center gap-4 flex-wrap">
                    ...
                    {!isRunning ? (
                        <button onClick={handleRun} className="self-end px-6 py-2 rounded-lg font-bold text-white bg-green-600 border-b-4 border-green-800 hover:bg-green-500 transition-colors">{t('scenario_runner.run_button')}</button>
                    ) : (
                        <button onClick={handleCancel} className="self-end px-6 py-2 rounded-lg font-bold text-white bg-red-600 border-b-4 border-red-800 hover:bg-red-500 transition-colors">{t('scenario_runner.cancel_button')}</button>
                    )}
                    <input type="file" ref={fileInputRef} onChange={handleFileChange} accept=".json" className="hidden" />
                    <button onClick={handleImportClick} disabled={isRunning} className="self-end px-6 py-2 rounded-lg font-bold text-white bg-blue-600 border-b-4 border-blue-800 hover:bg-blue-500 transition-colors disabled:bg-gray-500">{t('scenario_runner.import_button')}</button>
                    {importStatus && <span className={`self-end text-sm ${importStatus.includes('Error') ? 'text-red-400' : 'text-green-400'}`}>{importStatus}</span>}
                </div>
            </div>
            ...
            <div className="flex-grow bg-black/40 p-4 rounded-lg border border-cyan-700/50 overflow-y-auto">
                <div className="flex justify-between items-center mb-4">
                    ...
                    {results && <div className="flex items-center gap-2"> {/* ... Icons as before ... */} </div>}
                </div>
                {!results ? (
